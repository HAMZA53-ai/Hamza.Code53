      ...
      <div className="text-center p-6 sm:p-8 max-w-md w-full bg-[var(--panel-dark)] backdrop-blur-md rounded-lg border border-[var(--border-color)]">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          <button
            type="submit"
            disabled={!name.trim()}
            className="w-full h-11 sm:h-12 px-4 py-2 bg-[var(--neon-cyan)] text-black rounded-lg hover:shadow-[var(--glow-cyan)] disabled:bg-slate-600 disabled:text-slate-400 disabled:cursor-not-allowed disabled:shadow-none transition-all font-semibold text-base sm:text-lg"
          >
            ابدأ الآن
          </button>
        </form>
